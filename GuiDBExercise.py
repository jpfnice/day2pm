...
class MyWindow(tk.Tk):
    ...
    def insert(self):
        ...
        try:
            ...
            cursor.execute (f"insert into produit values ('{self.nameStr.get()}', {self.priceFloat.get()}, {self.qtyInt.get()})")
    ...
    def update(self):
        ...
        try:
            conn=MySQLdb.connect(host="sql7.freemysqlhosting.net", user="sql7372220", passwd="3sgiPGyWad", db="sql7372220")
            cursor=conn.cursor()
            cursor.execute (f"update produit set price={self.priceFloat.get()}, qty={self.qtyInt.get()} where name='{self.nameStr.get()}'")
            cursor.execute("commit")
            cursor.close()
            conn.close()
            self.onLoad() 
    ...
    def delete(self):
        ...
        try:
            conn=MySQLdb.connect(host="sql7.freemysqlhosting.net", user="sql7372220", passwd="3sgiPGyWad", db="sql7372220")
            cursor=conn.cursor()
            cursor.execute (f"delete from produit where name='{self.nameStr.get()}'")
            cursor.execute("commit")
            cursor.close()
            conn.close()
            self.onLoad() 
